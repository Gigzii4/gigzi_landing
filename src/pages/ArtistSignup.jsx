...
import { Mail, Lock, User, ArrowLeft, Brush } from 'lucide-react';
...
export default function ArtistSignup() {
  ...
  const { setToken, setUsername, setCity: setCityStore, setUser, setRole } = useAuthStore();
  ...
  const handleSignup = async () => {
    ...
    try {
      setLoading(true);
      const response = await postMethod('/v3/auth/artistSignup', { email, otp, name });
      ...
      if (response?.status || response?.data?.status || response?.success) {
        ...
        if (token) {
          try {
            ...
            setUsername(name);
            setCityStore(userData?.city || decoded.city || '');
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#021d5c] via-[#06328c] to-[#4D55CC] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="flex items-center mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-1">Artist Sign Up</h1>
            <p className="text-gray-600 text-sm">Join as an artist 🎨</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {!isEmailEntered ? (
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-[#021d5c] to-[#4D55CC] text-white py-3 rounded-full font-semibold hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Creating account...' : 'Sign Up as Artist'}
            </button>
          )}
