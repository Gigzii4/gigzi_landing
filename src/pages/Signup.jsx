...
export default function Signup() {
  ...
  const handleSignup = async () => {
    if (!email || !otp || !name || !city) {
      alert('Please fill all fields');
      return;
    }
    try {
      setLoading(true);
      const response = await postMethod('/v3/auth/signup', { email, otp, name, city });
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="space-y-6">
          ...
          {!isEmailEntered ? (
            <button
              onClick={sendOtp}
              disabled={otpLoading || !email || !name || !city}
          ...
          <div className="text-center space-y-2">
            <div>
              ...
              <Link to="/login" className="text-[#4D55CC] font-semibold hover:underline">
                Login
              </Link>
            </div>
            <div>
